...
unsigned char temp_buf[CONFIG_SYS_NAND_ECC_PAGE_SIZE]
...
static void nand_read_page(unsigned int real_addr, int syndrome, uint32_t *ecc_errors)
{
	uint32_t val;
	int ecc_off = 0;
	uint16_t ecc_mode = 0;
	...
	/* clear temp_buf */
	memset(temp_buf, 0, CONFIG_SYS_NAND_ECC_PAGE_SIZE);
	...
	page = real_addr / CONFIG_SYS_NAND_PAGE_SIZE;
	column = real_addr % CONFIG_SYS_NAND_PAGE_SIZE;

	if (syndrome) {
		column += (column / CONFIG_SYS_NAND_ECC_PAGE_SIZE) * ecc_off;
	}
	...
	writel((uint32_t)temp_buf,
	       SUNXI_DMA_BASE + SUNXI_DMA_DEST_START_ADDRR_REG0); /* read to RAM */
...
int nand_spl_load_image(uint32_t offs, unsigned int size, void *dest)
{
	...
	for (current_dest = dest;
			...
			current_dest += CONFIG_SYS_NAND_ECC_PAGE_SIZE) {
		nand_read_page(offs, offs < SYNDROME_PARTITIONS_END, &ecc_errors);
		count = current_dest - dest;

		if (size - count > CONFIG_SYS_NAND_ECC_PAGE_SIZE)
			current_count = CONFIG_SYS_NAND_ECC_PAGE_SIZE;
		else
		...
		memcpy(current_dest,
		       temp_buf,
		       current_count);
		offs += CONFIG_SYS_NAND_ECC_PAGE_SIZE;
	}
